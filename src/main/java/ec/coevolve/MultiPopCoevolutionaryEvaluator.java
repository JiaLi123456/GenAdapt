    ...
    public void setup( final EvolutionState state, final Parameter base,SearchRunner runner )
        ...
        if( numCurrent < 0 )
            ...
            for(int i = 0; i < numSubpopulations; i++)
                ...
                else selectionMethodCurrent[i].setup(state,base.push(P_SUBPOP).push(""+i).push(P_SELECTION_METHOD_CURRENT),runner);
                }
            }
        ...
        if( numPrev < 0 )
            ...
            for(int i = 0; i < numSubpopulations; i++)
                ...
                else selectionMethodPrev[i].setup(state,base.push(P_SUBPOP).push(""+i).push(P_SELECTION_METHOD_PREV),runner);
                }
            }
