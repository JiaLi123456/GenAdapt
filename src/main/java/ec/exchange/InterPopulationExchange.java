...
public class InterPopulationExchange extends Exchanger
    ...
    public void setup( final EvolutionState state, final Parameter _base ,SearchRunner runner)
        {
        base = _base;
        ...
        for( int i = 0 ; i < numsubpops ; i++ )
            ...
            exchangeInformation[i].immigrantsSelectionMethod.setup( state, p.push(P_SELECT_METHOD) ,runner);
            ...
            exchangeInformation[i].indsToDieSelectionMethod.setup( state, p.push(P_SELECT_TO_DIE_METHOD),runner);
