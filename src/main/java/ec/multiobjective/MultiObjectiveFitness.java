...
public class MultiObjectiveFitness extends Fitness
    ...
    */
    ...
    public void setObjectives(final EvolutionState state, double[] newObjectives)
        ...
        for (int i = 0; i < newObjectives.length; i++)
            ...
            if (_f >= Double.POSITIVE_INFINITY || _f <= Double.NEGATIVE_INFINITY || Double.isNaN(_f))
                ...
                {
                state.output.warnOnce(String.format("%s: The value of objective #%d is outside the expected bounds [%f, %f].  Did you configure the '%s' and '%s' parameters correctly?", this.getClass(), i, minObjective[i], maxObjective[i], P_MINOBJECTIVES, P_MAXOBJECTIVES));
                }
            }
    ...
    public void setup(EvolutionState state, Parameter base,SearchRunner runner)
        {
        super.setup(state, base,runner); // unnecessary really
